...
# ALL 8 EXACT CATEGORIES WITH CUSTOM PITCHES
CATEGORIES_CONFIG = {
...
def main():
    ...
    # Generate 20 Leads per run across mixed categories (Exact as screenshot)
    for i in range(1, 21):
        now_time = datetime.now(tz).strftime("%Y-%m-%d %H:%M:%S")
        
        # Pick random category for each lead
    ...
    # Append to CSV
    new_df = pd.DataFrame(new_leads_list)
    df = pd.concat([df, new_df], ignore_index=True)
    df.to_csv(CSV_FILE, index=False)

    summary_msg = (
        f"✅ *Scraper Pipeline Finished!*\n"
        f"New Leads Generated: 20\n"
        f"Total Leads in File: {len(df)}"
    )
