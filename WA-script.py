...
if weekday == 6:
    print("Sunday OFF. Exiting.")
    exit(0)

if weekday == 5 and current_hour >= 16:
    print("Saturday after 4 PM OFF. Exiting.")
    exit(0)
...
if not ID_INSTANCE or not API_TOKEN:
    print("❌ ERROR: Green API Credentials missing in Secrets!")
    exit(1)
...
def main():
    if not os.path.exists(CSV_FILE):
        print(f"❌ ERROR: {CSV_FILE} file missing!")
        return
    ...
    for idx, row in df.iterrows():
        ...
        # DUPLICATE PROTECTION: CHECKS HISTORY
        ...
        print(
            f"Sending ({sent_count + 1}/{max_messages}) to {b_name} ({clean_num})..."
        )
    ...
    print(f"Finished WhatsApp run. Sent: {sent_count}")
